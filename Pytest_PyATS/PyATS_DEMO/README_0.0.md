...
WorkFlow:
Workflow:
Routing Summary Log (CLI output)        [data/routing_summary/*.log]
        ↓
Raw Parser (CLI / regex based)           [src/utils/routing_summary_parser.py]
        ↓
EVPN Semantic Parser                     [src/semantic_parser/evpn_parser.py]
        ↓
Protocol-Level Facts (vendor-agnostic)   [semantic facts]
        ↓
EVPN Checker (rule-based evaluation)     [src/checkers/evpn.py]
        ↓
Operation YAML (remediation plan)        [operations/*.yaml]
